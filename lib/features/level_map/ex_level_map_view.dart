...
class ex_LevelMapView extends StatefulWidget {
  const ex_LevelMapView({Key? key}) : super(key: key);

  @override
  State<ex_LevelMapView> createState() => _ex_LevelMapViewState();
}

class _ex_LevelMapViewState extends State<ex_LevelMapView> {
        ...
        children: <Widget>[
          ...
          for (int index = 0; index < _positionLeft.length; index++)
            AnimatedPositioned(
              ...
              left: _position,
              top: (_isClick) ? _position: (context.sized.height-56)*.5,//(context.sized.height-56-context.sized.width*.3)*.5,
              child: Container(
...
setState(() {
  ...
  void zipla(){
    _isClick=!_isClick;
_position+=context.sized.height*.2;
print(_isClick);
setState(() {
});
  }
